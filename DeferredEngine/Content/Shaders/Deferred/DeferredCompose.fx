...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
	...
	float4 fog = volumeLightMap.Load(texCoordInt);

	float3 volumetrics = fog.rgb;
	float transmittance = saturate(fog.a);

	transmittance = max(transmittance, 0.2f);
	...
	float3 output = (finalValue * ssaoContribution * transmittance + volumetrics);
